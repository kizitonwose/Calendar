...
class ExamplesAdapter(val onClick: (ExampleItem) -> Unit) : RecyclerView.Adapter<ExamplesAdapter.BaseObjectViewHolder>() {

    val examples = mutableListOf<ExampleItem>().apply {
        add(ExampleItem(R.string.example_1, 0, HomeActivity::class.java))
        Unit
    }
    ...
    inner class BaseObjectViewHolder(override val containerView: View) :
        ...
        fun bind(item: ExampleItem) {
            ...
            itemOptionTitle.text = context.getString(item.titleRes)
            itemOptionSubtitle.text = context.getString(item.subtitleRes)
        }
    }
